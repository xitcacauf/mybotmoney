    ...
    try {
      ...
      if (command.ownerOnly && !checkPermission(message, "owner")) {
        return message
          .reply({
            embeds: [
              errorEmbed("Sem Permissão", "Apenas o dono pode usar este comando."),
            ],
          })
          .then((m) => setTimeout(() => m.delete().catch(() => {}), 5000));
      }
        ...
        return message
          .reply({
            embeds: [
              warningEmbed(
                "Cooldown",
                `Aguarde **${cd.timeLeft}s** antes de usar este comando novamente.`
              ),
            ],
          })
      ...
      logger.error(`Erro no messageCreate: ${err.message}`);
      message
        .reply({
          embeds: [
            errorEmbed("Erro Interno", "Ocorreu um erro ao executar o comando."),
          ],
        })
        .catch(() => {});
    }
