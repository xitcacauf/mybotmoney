...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    // max: 1 ensures collect fires at most once even if button is double-clicked
    ...
    collector.on("collect", async (i) => {
      if (i.customId === "divorce_confirm") {
        ...
        await User.findOneAndUpdate(
          ...
          { $set: { "relationship.status": "divorced", "relationship.partnerId": null, "relationship.partnerName": "" } }
        );
        await User.findOneAndUpdate(
        ...
        const successEmbed = new EmbedBuilder()
          ...
          .setDescription("O casamento foi dissolvido. Boa sorte na vida!");
        await i.update({ embeds: [successEmbed], components: [] });
      } else {
        await i.update({
          embeds: [new EmbedBuilder().setColor(config.colors.success).setTitle("✅ Cancelado").setDescription("Divórcio cancelado.")],
          components: [],
        });
      }
    });
